...
export default function Home() {
  return <main>
    ...
    <section className="catalog shell" id="hotels">
      ...
      <div className="hotel-grid">{hotels.map((hotel)=><article className="hotel-card reveal" key={hotel.name}><a className="hotel-photo" href={hotel.href} style={{backgroundImage:`url('${hotel.image}')`}} aria-label={`Подробнее: ${hotel.name}`}></a><div><p>{hotel.place}</p><h3>{hotel.name}</h3><span>{hotel.note}</span></div></article>)}</div>
    </section>
